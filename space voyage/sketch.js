var bg , bgImg , logo ,start,rules,ruletext,x,galaxy,galaxyImg;
...
function draw() {
  ...
  bg.velocityX=-(7+10-Math.round(score/8000));
  if(bg.x<500){
    bg.x=width/2;
  }
  if(gameState==="start"){
   ...
   if(gameState==="play"){
    ...
    fill(204,255,255);
    textFont("tisa");
    text("Distance remaining in miles:"+score ,220,40);
    //text("miles",850,40);
    text("Fuel remaining in %: " + fuel,230,585);
    //text("%",900,585);
    spawnObstacles();
    spaceStations();
    oGroup.scaleEach+=1;

    if(score<3000 && score>2950){
      galaxy=createSprite(displayWidth,displayHeight/2);
...
function spawnObstacles(){
  if(frameCount%50===0){
    ...
    if(r===10){
      obstacle.y=500;
      obstacle.velocityX=-Math.round(random(10,14));
    ...
    if(r===11){
      ...
      obstacle.x=1200;
      obstacle.velocityX=-Math.round(random(10,14));
     obstacle.velocityY=Math.round(random(5,7));
